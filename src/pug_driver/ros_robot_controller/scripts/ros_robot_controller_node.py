...
from ros_robot_controller.srv import GetBusServoState, GetPWMServoState, SetLegIK, SetLegFK, GetLegPosition, GetLegIKSim
from ros_robot_controller.msg import ButtonState, BuzzerState, LedState, MotorsState, BusServoState, SetBusServoState, SetPWMServoState, Sbus, OLEDState, SetPose, SetGait, JointAngle, SetJointAngle, Move, Stop
...
class ROSRobotController:
    ...
    def __init__(self, name):
        ...
        joint_angle_pub = rospy.Publisher('~robot/get_leg_ik_sim', JointAngle, queue_size=1)
        ...
        rospy.Service('~robot/set_leg_ik', SetLegIK, self.set_robot_leg_ik)
        rospy.Service('~robot/set_leg_fk', SetLegFK, self.set_robot_leg_fk)
        rospy.Service('~robot/get_leg_ik_sim', GetLegIKSim, self.set_robot_leg_ik_sim)
        rospy.Service('~robot/get_leg_position', GetLegPosition, self.get_robot_leg_position)
        rospy.Subscriber('~robot/set_pose', SetPose, self.set_robot_pose, queue_size=10)
        rospy.Subscriber('~robot/set_gait', SetGait, self.set_robot_gait, queue_size=10)
        rospy.Subscriber('~robot/set_joint_angle', SetJointAngle, self.set_robot_joint_angle, queue_size=10)
        rospy.Subscriber('~robot/move', Move, self.robot_move, queue_size=10)
        rospy.Subscriber('~robot/stop', Stop, self.robot_stop, queue_size=10)
        ...
        while not rospy.is_shutdown():
            if self.sim:
                self.pub_joint_data(joint_angle_pub)
    ...
    def set_robot_leg_fk(self, msg):
        position = self.board.set_leg_fk(np.array(msg.joint_angle).reshape((3, 4)))
        if position is not None:
            return [True, position.reshape(12).tolist()]
        else:
            return [False, []]

    def set_robot_leg_ik(self, msg):
        joint_angle = self.board.set_leg_ik(np.array(msg.position).reshape((3, 4)))
        if joint_angle is not None:
            return [True, joint_angle.reshape(12).tolist()]
        else:
            return [False, []]

    def set_robot_leg_ik_sim(self, msg):
        joint_angle = self.board.get_leg_ik_sim()
        if joint_angle is not None:
            return [True, joint_angle.reshape(12).tolist()]
        else:
            return [False, []]

    def pub_joint_data(self, pub):
        joint_angle = self.board.get_leg_ik_sim()
        if joint_angle is not None:
            pub.publish(joint_angle.reshape(12).tolist())

    def get_robot_leg_position(self, msg):
        result = self.board.get_leg_position()
        if result is not None:
            return [True, result.reshape(12).tolist()]
        else:
            return [False, []]

    def set_robot_pose(self, msg):
        self.board.set_pose(msg.x, msg.y, msg.z, msg.x_shift, msg.roll, msg.pitch, msg.duration)

    def set_robot_gait(self, msg):
        self.board.set_gait(msg.overlap_time, msg.swing_time, msg.clearance_time, msg.z_clearance)

    def set_robot_joint_angle(self, msg):
        self.board.set_joint_angle(np.array(msg.joint_angle).reshape((3, 4)), msg.duration)

    def robot_move(self, msg):
        self.board.move(msg.x, msg.y, msg.angle)

    def robot_stop(self, msg):
        self.board.stop(msg.duration)
