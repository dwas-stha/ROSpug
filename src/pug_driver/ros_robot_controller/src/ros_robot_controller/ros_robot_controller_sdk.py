...
class Board:
    ...
    def __init__(self, device="/dev/rrc", baudrate=500000, robot=1, timeout=5):
        ...
        self.kinematics_read_lock = threading.Lock()
        ...
        self.leg_position_queue = queue.Queue(maxsize=1)
        ...
        self.parsers = {
            ...
            PacketFunction.PACKET_FUNC_KINEMATICS: self.packet_report_leg_position,
        }

        time.sleep(0.5)
        self.set_robot_type(robot)
        threading.Thread(target=self.recv_task, daemon=True).start()

    def packet_report_leg_position(self, data):
        try:
            self.leg_position_queue.put_nowait(data)
        except queue.Full:
            pass
    ...
    def get_position(self):
        ...
        if self.enable_recv:
            try:
                data = self.leg_position_queue.get(block=False)
                if data[0] == 0x17:
                    # print(len(data))
                    cmd, *info = struct.unpack("<13f", data)
                    info = np.array(info).reshape((3, 4))
                    return info
                else:
                    None
            except queue.Empty:
    ...
    def buf_write(self, func, data):
        ...
        hexadecimal_list = [hex(num) for num in buf]
    ...
    def set_leg_fk(self, joint_angle):
        ...
        [ 0,     -0.01,  -0.01,  -0.01]  X
        [ 0.0,    0.0,    0.0,    0.0]   Y
        [-0.15,  -0.15,  -0.15,  -0.15]  Z 
        '''
        if self.enable_recv:
            data = [0x01]  
            data.extend(struct.pack("<12f", joint_angle[0, 0], joint_angle[0, 1], joint_angle[0, 2], joint_angle[0, 3],
                               joint_angle[1, 0], joint_angle[1, 1], joint_angle[1, 2], joint_angle[1, 3],
                               joint_angle[2, 0], joint_angle[2, 1], joint_angle[2, 2], joint_angle[2, 3]))
            with self.kinematics_read_lock:
                self.buf_write(PacketFunction.PACKET_FUNC_KINEMATICS, data)
                try:
                    data = self.leg_position_queue.get(block=True, timeout=1)
                except:
                    return None
                # B：表示一个无符号字节（8位），范围是0到255。
                # b：表示一个有符号字节（8位），范围是-128到127。
                # h -32768~32767
                # H 0~65535
                cmd, *info = struct.unpack("<B12f", data)
                info = np.array(info).reshape((3, 4))
                return info
            # time.sleep(0.004)
        else:
    ...
    def set_leg_ik(self, foot_locations):
        ...
        if self.enable_recv:
            data = [0x03]  
            data.extend(struct.pack("<12f", foot_locations[0, 0], foot_locations[0, 1], foot_locations[0, 2], foot_locations[0, 3],
                               foot_locations[1, 0], foot_locations[1, 1], foot_locations[1, 2], foot_locations[1, 3],
                               foot_locations[2, 0], foot_locations[2, 1], foot_locations[2, 2], foot_locations[2, 3]))
            with self.kinematics_read_lock:
                self.buf_write(PacketFunction.PACKET_FUNC_KINEMATICS, data)
                try:
                    data = self.leg_position_queue.get(block=True, timeout=1)
    ...
    def set_joint_angle(self, joint_angle, duration=0):
        # 驱动舵机转到指定角度, 输入3x4数组和运行时间s
        '''
         FR     FL    BR    BL
        [1,     4,    7,    10]  calf
        [2,     5,    8,    11]  thigh
        ...
        '''
        data = [0x07]
        data.extend(struct.pack("<f12f", duration, joint_angle[0, 0], joint_angle[0, 1], joint_angle[0, 2], joint_angle[0, 3], 
                                joint_angle[1, 0], joint_angle[1, 1], joint_angle[1, 2], joint_angle[1, 3], 
                                joint_angle[2, 0], joint_angle[2, 1], joint_angle[2, 2], joint_angle[2, 3]))
        self.buf_write(PacketFunction.PACKET_FUNC_KINEMATICS, data)
        # time.sleep(0.004)

    def set_pose(self, x, y, z, x_shift, roll, pitch, duration=0.0):
        # 更新姿态, 每条腿x，y，z的位置, roll和pitch姿态
        # 坐标是以大腿关节为原点
        data = [0x09]
        data.extend(struct.pack("<7f", x, y, z, x_shift, pitch, roll, duration))
        self.buf_write(PacketFunction.PACKET_FUNC_KINEMATICS, data)
        # time.sleep(0.004)

    def set_gait(self, overlap_time, swing_time, clearance_time, z_clearance):
        # 更新步态 
        # overlap_time: 4脚全部着地的时间s
        # swing_time： 单脚离地时间s
        # clearance_time: 前后交叉脚相位间隔时间s
        # z_clearance： 步高m
        data = [0x0B]
        data.extend(struct.pack("<4f", overlap_time, swing_time, clearance_time, z_clearance))
        self.buf_write(PacketFunction.PACKET_FUNC_KINEMATICS, data)
        # time.sleep(0.004)

    def move(self, x_move_amplitude, y_move_amplitude, angle_move_amplitude):
        # 移动, 单位m
        data = [0x0D]
        data.extend(struct.pack("<3f", x_move_amplitude, y_move_amplitude, angle_move_amplitude))
        self.buf_write(PacketFunction.PACKET_FUNC_KINEMATICS, data)
        # time.sleep(0.004)
    ...
    def stop(self, duration):
        ...
        # 停止, 运行时间s
        data = [0x0F]
        data.extend(struct.pack("<1f", duration))
        self.buf_write(PacketFunction.PACKET_FUNC_KINEMATICS, data)
        # time.sleep(0.004)

    def set_leg_ik_sim(self, foot_locations):
        # urdf模型关节, 输入四个落脚点位置, 返回每个关节的角度，坐标是以大腿关节为原点, 输入为3x4的数组
        '''
          FR      FL      BR      BL
        [ 0,     -0.01,  -0.01,  -0.01]  X
        [ 0.0,    0.0,    0.0,    0.0]   Y
    ...
    def set_robot_type(self, robot=0):
        # 0 nothing
        # 1 rospug
        data = [0x15]
        data.extend(struct.pack("<B", robot))
        self.buf_write(PacketFunction.PACKET_FUNC_KINEMATICS, data)
    ...
    def recv_task(self):
        while True:
            if self.enable_recv:
                ...
                with self.lock:
                    if self.close_port and self.port is not None:
                        self.port.close()
                        self.port = None
...
def kinematics_test(board):
    # FR    FL    BR     BL
    foot_locations_init = np.array([[0.01, 0.01, 0.01, 0.01],  # X
                               [ 0.,   0.,   0.,   0.],   # Y
                               [-0.13,  -0.13,  -0.13,  -0.13]    # Z
                               ])

    res = board.set_leg_ik(foot_locations_init)
    print(res)
    print(board.set_leg_fk(np.array([[500, 500, 500, 500],
                                     [500, 500, 500, 500],
                                     [500, 500, 500, 500]])))
    board.set_joint_angle(res, 0.5)
    time.sleep(0.5)

    board.set_pose(0, 0, -0.13, 0, 0, 0, 0.5)
    time.sleep(1)
    print(board.get_leg_position())
    board.set_pose(0, 0, -0.15, 0, 0, 0, 0.5)
    time.sleep(1)
    print(board.get_leg_position())

    board.set_pose(0, -0.0, -0.13, 0.004, 0, 0, 0.5)
    time.sleep(1)
    board.set_gait(0.2, 0.18, 0.02, 0.04)
    board.move(0.13, 0, 0)
    time.sleep(3)
    board.stop(0)
...
if __name__ == "__main__":
    ...
    # kinematics_test(board)
    # print(board.bus_servo_read_id())
    # board.bus_servo_set_position(1, [[1, 500]])
    # time.sleep(0.5)
