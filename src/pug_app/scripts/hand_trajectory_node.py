...
from pug_app.msg import Points, PixelPosition
from ros_robot_controller.msg import BuzzerState
from utils.utils import rotate_and_crop_rectangle
from std_srvs.srv import Trigger, TriggerResponse
from pug_sdk.common import vector_2d_angle, distance
...
class HandTrajectoryNode:
    def __init__(self, name):
        ...
        self.result_publisher = rospy.Publisher('/hand_gesture/image_result', Image, queue_size=1)  # 图像处理结果发布
    ...
    def enter_func(self, msg):
        if self.image_sub is None: 
            self.image_sub = rospy.Subscriber('/csi_camera/image_rect_color', Image, self.image_callback)  # 摄像头订阅
        self.enter = True
            ...
            if self.enter:
                if self.image is not None:
                    ...
                    if self.start:
                        ...
                        rospy.sleep(0.01)
                    self.result_publisher.publish(cv2_image2ros(cv2.resize(bgr_image, (640, 360)), self.name))
