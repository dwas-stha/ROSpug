...
from pug_example.msg import ObjectInfo, ObjectsInfo
...
class Yolov5Node:
    def __init__(self, name):
        ...
        self.fps = common.FPS()  # fps计算器
        engine = rospy.get_param('~engine', 'traffic_signs_640s_7_0.engine')
        lib = rospy.get_param('~lib', 'libmyplugins.so')
        conf_thresh = rospy.get_param('~conf_thresh', 0.8)
        self.classes = rospy.get_param('~classes', ['go', 'right', 'park', 'red', 'green', 'crosswalk'])
    ...
    def image_proc(self):
        while self.running:
            ...
            self.result_image_pub.publish(cv2_image2ros(result_image, frame_id='yolov5'))
        else:
            rospy.sleep(0.01)
        self.yolov5.destroy() 
